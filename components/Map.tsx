  ...
  const fetchLugares = useCallback(async () => {
    ...
    try {
      ...
      if (data.geojson?.features) {
        ...
        // Abrir panel automáticamente si hay búsqueda
        if (filters.buscar) setPanelOpen(true)
      }
  ...
  return (
    <div className="relative w-full h-full flex">
      ...
      <div className="flex-1 relative">
        ...
        {/* Filtros */}
        <div className="absolute top-16 left-4 z-[1000] flex flex-col gap-2 pointer-events-auto">
        ...
        <div className="absolute bottom-8 left-4 z-[1000]">
          <div className="bg-[#0D1B2A]/90 backdrop-blur border border-white/10 rounded-xl py-2 px-4 flex gap-6 text-sm">
        ...
        {!panelOpen && (
          <button
            ...
            className="absolute top-4 right-4 z-[1000] bg-[#0D1B2A]/90 backdrop-blur border border-white/10 rounded-xl px-3 py-2 text-xs text-white/60 hover:text-white hover:border-white/20 transition-colors flex items-center gap-2"
          >
            <span>☰</span> Lista
          </button>
        )}
      ...
      {/* ── Panel lateral ── */}
      {panelOpen && (
        <div className="w-80 shrink-0 relative z-[1000] flex flex-col">
          {/* Botón cerrar panel */}
          <button
            onClick={() => { setPanelOpen(false); setSelectedLugar(null) }}
            className="absolute top-2 right-2 z-10 text-white/30 hover:text-white transition-colors text-lg leading-none"
          >
            ×
          </button>
          <LugaresPanel
