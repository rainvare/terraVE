...
// Panel lateral con lista de lugares con foto + card de detalle
...
function DetailCard({ lugar, onClose, googleMapsKey }: {
  lugar: Lugar
  onClose: () => void
  googleMapsKey: string
}) {
  ...
  // Extraer dirección del campo descripcion
  const direccion = lugar.descripcion
    ? lugar.descripcion.split(' | ')[1] ?? ''
    : ''
    ...
    <div className="flex flex-col h-full">
      ...
      <div className="flex items-start justify-between gap-2 p-4 border-b border-white/10">
        <div className="min-w-0">
          ...
          {direccion && (
            <p className="text-xs text-white/35 mt-1 line-clamp-2">{direccion}</p>
          )}
        </div>
        <button
          onClick={onClose}
          className="text-white/30 hover:text-white transition-colors text-xl leading-none shrink-0 mt-0.5"
        >
          ←
        </button>
      </div>
      ...
      <div className="px-4 py-3 border-b border-white/10">
        ...
        >
          ...
          {lugar.reporte?.confianza
            ? ` · ${(lugar.reporte.confianza * 100).toFixed(0)}% confianza`
            : ''}
        </span>
      </div>
      ...
      <div className="grid grid-cols-2 divide-x divide-white/10 border-b border-white/10">
        ...
        <div className="flex flex-col">
          ...
          {fotoAntes ? (
            <div className="relative">
              <img src={fotoAntes} alt="antes" className="w-full h-40 object-cover" />
              <span className="absolute bottom-1 right-1 text-[9px] bg-black/60 text-white/60 px-1.5 py-0.5 rounded">
                Google Maps
              </span>
            </div>
          ) : (
            <div className="h-40 bg-white/5 flex flex-col items-center justify-center text-white/20 text-xs gap-1">
              <span className="text-2xl">🗺️</span>
              <span>Sin foto previa</span>
            </div>
          )}
        </div>
        ...
        <div className="flex flex-col">
          ...
          {fotoDespues ? (
            <img src={fotoDespues} alt="después" className="w-full h-40 object-cover" />
          ) : (
            <div className="h-40 bg-white/5 flex flex-col items-center justify-center text-white/20 text-xs gap-1">
        ...
        {lugar.reporte?.created_at && (
          <div className="flex justify-between">
            ...
            <span className="text-white/60">
              {new Date(lugar.reporte.created_at).toLocaleDateString('es-VE')}
            </span>
          </div>
        )}
      </div>
      ...
      <div className="px-4 py-3 flex gap-3">
        ...
        <a
          href={`/classify?lugar_id=${lugar.id}`}
...
function LugarItem({ lugar, onClick }: { lugar: Lugar; onClick: () => void }) {
  ...
  const fotoDespues = lugar.reporte?.foto_despues || null
  const fotoAntes   = lugar.foto_antes ?? null
  const foto        = fotoAntes ?? fotoDespues
  ...
  return (
    ...
    >
      ...
      <span
        ...
        style={{ background: SEMAFORO_COLOR[color] }}
      />
    </button>
  )
}
        ...
        <>
          ...
          <div className="px-4 pt-4 pb-3 border-b border-white/10">
            <p className="text-xs text-white/40">
              {lista.length} lugar{lista.length !== 1 ? 'es' : ''}
              {tab === 'con_foto' && ` con foto`}
            </p>
