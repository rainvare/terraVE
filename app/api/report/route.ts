...
function FotoUpload({
  label, hint, preview, onChange, required
}: {
  label: string
  hint: string
  preview: string | null
...
export default function ReportForm() {
  ...
  const [duplicados,  setDuplicados]       = useState<{id: string, nombre: string, lat: number, lng: number}[]>([])
  ...
  const buscarDuplicados = async (nombre: string) => {
    ...
    if (data.geojson?.features) {
      const coincidencias = data.geojson.features
        .map((f: any) => ({ id: f.properties.id, nombre: f.properties.nombre, lat: f.geometry.coordinates[1], lng: f.geometry.coordinates[0] }))
  ...
  const handleSubmit = async () => {
    ...
    if (duplicados.length > 0 && !ignorarDup) {
      setError('Ya existe un lugar con ese nombre. ¿Es el mismo? Si no, hacé clic en "Registrar igual".')
      return
    }
    ...
    try {
      ...
      // 2. Si hay foto de daño actual, crear reporte
  ...
  return (
    <div className="max-w-lg mx-auto space-y-6">
      ...
      {duplicados.length > 0 && !ignorarDup && (
        <div className="bg-[#D4A017]/10 border border-[#D4A017]/30 rounded-xl p-4 flex flex-col gap-3">
      ...
      {/* Foto actual del daño */}
      <FotoUpload
        label="Foto del daño actual"
        hint="Sacá una foto ahora del estado del edificio"
        preview={fotoDespues}
