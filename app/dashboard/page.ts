    ...
    async function cargar() {
      try {
        const [sRes, mRes, cRes] = await Promise.all([
          ...
          fetch(`${SUPABASE_URL}/rest/v1/poblacion_expuesta?zona_critica=eq.true&order=pop_total_expuesta.desc&limit=15&select=adm1_name,adm2_name,pop_total_expuesta,pop_mmi_viii,pop_mmi_ix,zona_critica`, { headers }),
      ...
      <main className="flex-1 px-4 py-6 max-w-4xl mx-auto w-full">
        <div className="mb-6">
          ...
          <p className="text-xs text-white/40 mt-1">
            Análisis cruzado · Sentinel-1 SAR + Reportes ciudadanos + UNOSAT · 24 jun 2026
          </p>
        </div>
          ...
          <>
            ...
            <section className="mb-6">
              ...
              <div className="grid grid-cols-2 gap-3 sm:grid-cols-4">
                <StatCard
                  label="Área afectada"
                  value={`${Number(stats?.area_danada_km2 ?? 0).toFixed(1)} km²`}
                  sub="Detección SAR"
                  color="text-red-400"
                />
                <StatCard
                  label="Coincidencias"
                  value={stats?.coincidencias ?? 0}
                  sub="SAR + ciudadano"
                  color="text-[#D4A017]"
                />
                <StatCard
                  label="Población crítica"
                  value={Number(stats?.poblacion_critica ?? 0).toLocaleString()}
                  sub="Intensidad VII+"
                  color="text-orange-400"
                />
                <StatCard
                  label="Municipios críticos"
                  value={stats?.municipios_criticos ?? 0}
                  sub="Zona VII o superior"
                  color="text-blue-400"
                />
              </div>
            </section>
            ...
            <section className="mb-6">
              <div className="bg-[#0D1B2A] border border-white/10 rounded-xl p-4">
                <p className="text-xs text-white/40 uppercase tracking-wider mb-1">Cobertura ciudadana</p>
                <div className="flex items-center gap-4 mt-3">
                  <div className="flex-1 bg-white/5 rounded-full h-2">
                    <div
                      ...
                      style={{
                        width: `${stats
                          ? Math.min(100, (stats.zonas_con_reporte / (stats.zonas_satelitales || 1)) * 100 * 1000)
                          : 0}%`
                      }}
                    />
                  </div>
                  <p className="text-xs text-white/50 whitespace-nowrap">
                    {stats?.zonas_con_reporte ?? 0} de {stats?.zonas_satelitales ?? 0} zonas SAR con reporte
                  </p>
                </div>
                <p className="text-xs text-white/30 mt-2">
                  ⚠️ {Number(stats?.zonas_satelitales ?? 0) - Number(stats?.zonas_con_reporte ?? 0)} zonas de daño satelital sin ningún reporte ciudadano todavía
                </p>
              </div>
            </section>
            ...
            {coincidencias.length > 0 && (
              <section className="mb-6">
                <p className="text-xs text-white/40 uppercase tracking-wider mb-3">
                  Edificios con daño confirmado (SAR + ciudadano)
                </p>
                      ...
                      {coincidencias.map((c, i) => (
                        <tr key={i} className="border-b border-white/5 hover:bg-white/5">
                          ...
                          <td className="px-4 py-2 text-right text-white/40">
                            {c.area_m2 ? `${(c.area_m2).toFixed(0)} m²` : '—'}
                          </td>
                        </tr>
                      ))}
                    </tbody>
            ...
            <section className="mb-6">
              <p className="text-xs text-white/40 uppercase tracking-wider mb-3">
                Municipios con mayor exposición (intensidad VII+)
              </p>
                  ...
                  return (
                    <div key={i} className="bg-[#0D1B2A] border border-white/10 rounded-xl px-4 py-3">
                      ...
                      <div className="bg-white/5 rounded-full h-1.5">
                        <div
                          className="bg-orange-500/60 h-1.5 rounded-full"
                          style={{ width: `${pct}%` }}
                        />
                      </div>
                      {(m.pop_mmi_viii > 0 || m.pop_mmi_ix > 0) && (
